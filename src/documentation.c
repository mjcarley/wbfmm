 ...
 * @param normals normals at source points (can be NULL);
 * @param nstr stride between normals in bytes;
 * @param npts the number of points in \a pts;
 * @param sorted if TRUE, the source points are assumed to be sorted in 
 * Morton index order and any source data used in subsequent calculations
 * likewise.
 * 
 * @return 0 on success.
 */
 ...
 * @param th rotation angle \f$\theta\f$, from wbfmm_rotation_angles;
 ...
 * ::wbfmm_coefficients_H_rotation, rotate input coefficients to
 * new system of axes, using \a H and angles \f$\phi\f$ and
 * \f$\chi\f$. Input and output are strided arrays of dense complex
 * data with spacing between adjacent complex values given as \a cstri
 * and \a cstro elements respectively. Thus, \a Co for example is
 * packed as:
 *
 ...
 * wbfmm_rotate_H_avx which uses AVX optimizations if
 * available. The compile time switch -DWBFMM_USE_AVX selects the AVX
 * version.
 *
 ...
 * @param sc scaling of \a Co before adding rotated coefficients
 * (usually 0 or 1). 
 *
 * @return 0 on success
 */
 ...
 * @param sc scaling applied to Co before addition of translated \a Ci.
 *
 * @return 0 on success
 */
 ...
 * @param dipoles dipole source strengths;
 ...
 * @param sstr stride of data in \a src;
 * @param d dipole strengths for vector sources;
 ...
 * ::wbfmm_child_parent_shift. Note that the rotation matrices
 * for this function are switched relative to the rotations of the
 * same name in ::wbfmm_child_parent_shift, because the `upper'
 ...
 * should be initialized with ::wbfmm_shift_angle_table_init
 *
 ...
 * ::wbfmm_shift_operators_new, in order to initialize the
 * look-up table of orientations between boxes in interaction lists.
 *
 * @return 0 on success
 *
 */
...
*wbfmm_shift_operators_new(guint L, gboolean bw, gdouble *work) {}
 ...
 *
 * The function is available as a reference version
 * wbfmm_downward_pass_ref and an optimized version
 * wbfmm_downward_pass_avx which uses AVX optimizations if
 * available. The compile time switch -DWBFMM_USE_AVX selects the AVX
 * version. The parameter \a nthreads is ignored if the library does
 * not have OpenMP support. Otherwise, if \a nthreads=0, the
 * unthreaded version is used, if \a nthreads>0 it is treated as the
 * number of threads, and if \a nthreads<0, the number of threads is
 * determined by the CPU (usually the number of processors).
 *
 ...
 * @param work workspace;
 * @param nthreads number of threads to use in OpenMP.
 *
 * @return 0 on success
 *
 */
 ...
 *
 */
 gint wbfmm_bessel_h_init(gdouble x, gdouble *h0,
				gdouble *h1) {}
...
gint wbfmm_laplace_expansion_normal_cfft(gint N,
					       gdouble *x0,
					       gdouble *xs,
					       gdouble
					       *normal
...
gint wbfmm_laplace_expansion_evaluate(gdouble *x0,
					    gdouble *cfft,
					    gint cstr, gint N,
					    gint nq,
					    gdouble *xf,
...
gint wbfmm_laplace_expansion_grad_evaluate(gdouble *x0,
						 gdouble *cfft,
						 gint cstr,
						 gint N,
						 gint nq,
 ...
 * @param sc scaling factor to premultiply \a Co.
 * 
 * @return 0 on success
 *
 **/
 ...
 * wbfmm_laplace_local_coefficients for the field (singular) or
 * local (regular) expansions respectively.
 * 
 ...
 * @param N order of expansion;
 * @param field field specifier using ::wbfmm_field_t;
 ...
 * @param src source strengths;
 * @param sstr stride of data in \a src;
 * @param d dipole source strengths; 
 * @param dstr stride in \a d;
 * @param eval_neighbours if TRUE compute contributions from sources 
 * in box \a b and neighbours; 
 * @param work workspace.
 ...
 * ::wbfmm_tree_add_points and indexing in the array must
 * correspond to that in the point list. Input arrays may be NULL: if
 * \a src is not NULL, it is interpreted as a list of monopole
 * strengths; if \a normals is not NULL, \a dipoles may not be NULL
 * and they are interpreted respectively as a vector (`normal') at
 * each source position and a scalar amplitude (this corresponds to
 * surface normal and a normal velocity amplitude in a boundary
 * element method calculation); if \a normals is NULL and \a dipoles
 * is not NULL, \a dipoles is interpreted as a three-element vector
 * specifying the dipole strength. The strides \a sstr, \a nstr, and
 * \a dstr are the number of scalar elements between successive
 * entries in the arrays, with the elements of each entry densely
 * packed. For example, a list of normals might read:
 *
 * \f$[n_{x1}\quad n_{y1}\quad n_{z1}\quad a_{1}\quad b_{1}\quad n_{x2}
 * \ldots]\f$
 *
 ...
 * @ingroup laplace
 *
 * @brief Initialize expansion coefficient data for the Laplace problem 
 * in an octree 
 *
 * @param t octree for problem;
 * @param l level to initialize data for;
...
gint wbfmm_laplace_field_grad(gdouble *xs,
				    gint xstride,
				    gdouble *src,
				    gint sstride,
				    gint nq,
 ...
 * @ingroup laplace
 *
 * @brief Direct evaluation of the curl of a Laplace field from a
 * list of sources
 * 
 * @param xs source positions;
 * @param xstride source array stride;
 * @param src source strengths;
 * @param sstride source strength array stride;
 * @param nq number of source components (must be at least three);
 * @param normals normals at source positions;
 * @param nstr normal stride;
 * @param dipoles dipole components;
 * @param dstr dipole stride;
 * @param nsrc number of sources;
 * @param xf field point;
 * @param field on output incremented with computed curl;
 * @param fstr stride in \a field (ignored).
 *
 * @return 0 on success
 *
*/

gint wbfmm_laplace_field_curl(gdouble *xs,
				    gint xstride,
				    gdouble *src,
				    gint sstride,
				    gint nq,
 ...
 * @ingroup laplace
 *
 * @brief Evaluate local Laplace expansion
 * 
 * @param x0 centre of expansion;
 * @param cfft local expansion coefficients;
 * @param cstr stride in \a cfft;
 * @param N order of expansion;
 * @param nq number of source components;
 * @param xf field point;
 ...
 * @ingroup laplace
 *
 * @brief Evaluate gradient of local Laplace expansion
 *
 * Evaluate the gradient \f$\nabla\phi_{i}\f$ for source components 
 * \f$i=0,\ldots n_{q}-1\f$, packing gradient vectors in \a field. 
 * 
 * @param x0 centre of expansion;
 * @param cfft local expansion coefficients;
 * @param cstr stride in \a cfft;
 ...
 * @param xf field point;
 * @param field field value;
 * @param fstr stride between start of gradient vectors (must be at least 3);
 * @param work workspace.
 *
 * @return 0 on success
 *
...
gint
wbfmm_laplace_expansion_local_grad_evaluate(gdouble *x0,
						  gdouble *cfft,
						  gint cstr, 
						  gint N,
						  gint nq,
 ...
 * @ingroup laplace
 *
 * @brief Evaluate curl of local Laplace expansion
 *
 * Evaluate the curl \f$\nabla\times\phi\f$ for first three components
 * in expansion, returning curl vector in field.
 * 
 * @param x0 centre of expansion;
 * @param cfft local expansion coefficients;
 * @param cstr stride in \a cfft;
 * @param N order of expansion;
 * @param nq number of source components (must be at least three, first
 * three terms are taken as vector field whose curl is to be taken);
 * @param xf field point;
 * @param field field value;
 * @param fstr ignored;
 * @param work workspace.
...
gint
wbfmm_laplace_expansion_local_curl_evaluate(gdouble *x0,
						  gdouble *cfft,
						  gint cstr, 
						  gint N,
						  gint nq,
 ...
 * @ingroup laplace
 *
 * Evaluate curl of field for Laplace problem for point in a specified
 * box.
 * 
 * @param t octree for problem;
 * @param level box level in tree;
 * @param b index of box containing expansion;
 * @param x evaluation point;
 * @param f on output contains evaluated field;
 * @param fstr stride in \a f;
 * @param src input source terms;
 * @param sstr stride in \a src;
 * @param d dipole strengths for vector sources;
 * @param dstr stride in \a d;
 * @param eval_neighbours if TRUE, add direct contribution from sources
 * in neighbour boxes;
 * @param work workspace.
 * 
 * @return 0 on success.
...
gint
wbfmm_tree_laplace_box_local_curl(wbfmm_tree_t *t,
					guint level,
					guint b,
					gdouble *x,
					gdouble *f,
 ...
 *
 * The function is available as a reference version
 * wbfmm_laplace_downward_pass_ref and an optimized
 * version wbfmm_laplace_downward_pass_avx which
 * uses AVX optimizations if available. The compile time switch
 * -DWBFMM_USE_AVX selects the AVX version. The parameter \a nthreads
 * is ignored if the library does not have OpenMP support. Otherwise,
 * if \a nthreads=0, the unthreaded version is used, if \a nthreads>0
 * it is treated as the number of threads, and if \a nthreads<0, the
 * number of threads is determined by the CPU (usually the number of
 * processors). Threading is not implemented on the reference version.
 *
 * @param t an initialized octree which has had the upward pass performed;
 * @param op shift operators allocated for \a t;
 * @param level level at which to perform downward pass;
 ...
 *
 * @brief Generate coefficients for evaluation of local field from
 * (regular) expansion coefficients in the Helmholtz problem. The
 * coefficients from this function can be applied to an expansion
 * using ::wbfmm_expansion_apply
 * 
 * @param k wavenumber;
 * @param x location of evaluation point relative to centre of expansion;
 * @param N order of expansion;
...
gint wbfmm_local_coefficients(gdouble k,
				    gdouble *x,
				    gint N,
				    guint field,
				    gdouble *cfft,
 ...
 *
 * @brief Apply evaluation coefficients to coefficients of an
 * expansion to evaluate the Helmholtz potential.
 * 
 * @param C coefficients of expansion;
 * @param cstr stride in \a C;
 * @param nq number of source terms in \a C;
...
gint wbfmm_expansion_apply(gdouble *C,
				 gint cstr,
				 gint nq,
				 gdouble *ec,
				 gint N,
 ...
 * @ingroup targets
 *
 * @brief Allocate a target list structure for efficient evaluation of
 * fields
 * 
 * @param t tree to which target list is connected;
 * @param npts maximum number of target points
 *
 * @return newly allocated :wbfmm_target_list_t for the requested
 * number of target points
 *
*/

wbfmm_target_list_t *wbfmm_target_list_new(wbfmm_tree_t *t,
						 guint npts) ;

/** 
 * @ingroup laplace
 *
 * Evaluate the field at a point in a given tree box for the Laplace
 * problem using FMM
 * 
 * @param t a ::wbfmm_tree_t;
 * @param level the level of \a b within \a t;
 * @param b the index of a box at level \a level of \a t;
 * @param src array of source strengths for points in \a t;
 * @param sstr stride in \a src;
 * @param d dipole strengths in \a t;
 * @param dstr stride in \a d;
 * @param field field to be evaluated, XOR of ::wbfmm_field_t;
 * @param eval_neighbours if TRUE, include direct contributions from
 * boxes neighbouring \a b;
 * @param x evaluation point for field;
 * @param f on output incremented by field at \a x;
 * @param fstr stride between components of field in \a f;
 * @param work workspace.
 * 
 * @return 0 on success.
 */
 ...
 * 
 * Evaluate the field at a point in a given tree box for the Laplace
 * problem, by direct summation
 *
 * @param xs source points
 * @param xstr stride in \a xs;
 * @param n normals;
 * @param nstr stride in \a n;
 * @param nsrc number of sources;
 * @param src array of source strengths for points in \a t;
 * @param sstr stride in \a src;
 * @param d dipole strengths in \a t;
 * @param dstr stride in \a d;
 * @param nq number of components of sources;
 * @param field field to be evaluated, XOR of ::wbfmm_field_t;
 * @param xf evaluation point for field;
 * @param f on output incremented by field at \a x;
 * @param fstr stride between components of field in \a f;
 * 
 * @return 0 on success.
...
gint WBFMM_FUNCTION_NAME(wbfmm_laplace_field_direct)(gdouble *xs,
						     gint xstr,
						     gdouble *n,
						     gint nstr,
						     gint nsrc,
 ...
 * @param H on output rotation coefficients;
 * @param N maximum order of coefficients to compute;
 * @param th rotation angle \f$\theta\f$, from wbfmm_rotation_angles_f;
 * @param work workspace
 *
 * @return 0 on success
 */
 ...
 * 
 * Given the rotation coefficients \a H for angle \f$\theta\f$ from
 * ::wbfmm_coefficients_H_rotation_f, rotate input coefficients to
 * new system of axes, using \a H and angles \f$\phi\f$ and
 * \f$\chi\f$. Input and output are strided arrays of dense complex
 * data with spacing between adjacent complex values given as \a cstri
 * and \a cstro elements respectively. Thus, \a Co for example is
 ...
 * wbfmm_rotate_H_avx_f which uses AVX optimizations if
 * available. The compile time switch -DWBFMM_USE_AVX selects the AVX
 * version.
 *
 * @param Co on output contains rotated coefficients;
 ...
 * Shift the (regular) expansion data from a parent box to each of its
 * child boxes, assuming the same packing as in
 * ::wbfmm_child_parent_shift_f. Note that the rotation matrices
 * for this function are switched relative to the rotations of the
 * same name in ::wbfmm_child_parent_shift_f, because the `upper'
 * children rotate `down' to be shifted to the parent centre but the
 * rotation is `up' to shift from the parent to those children, and
 * similarly for the `lower' children.
 *
 ...
 * Find the rotation angles \f$(\theta,\phi\,\chi)\f$ between a box at
 * integer coordinates \f$(i,j,k)\f$, using a look-up table which
 * should be initialized with ::wbfmm_shift_angle_table_init_f
 *
 * @param i integer \f$x\f$ coordinate of box on interaction list;
 * @param j integer \f$y\f$ coordinate of box on interaction list;
 * @param k integer \f$z\f$ coordinate of box on interaction list;
 ...
 * This function must be called before any interaction calculations
 * are performed, in particular before any call to
 * ::wbfmm_shift_operators_new_f, in order to initialize the
 * look-up table of orientations between boxes in interaction lists.
 *
 * @return 0 on success
 *
...
wbfmm_shift_operators_t
*wbfmm_shift_operators_new_f(guint L, gboolean bw, gfloat *work) {}
...
gint wbfmm_shift_operators_coaxial_SR_init_f
 ...
 *
 * The function is available as a reference version
 * wbfmm_downward_pass_ref_f and an optimized version
 * wbfmm_downward_pass_avx_f which uses AVX optimizations if
 * available. The compile time switch -DWBFMM_USE_AVX selects the AVX
 * version. The parameter \a nthreads is ignored if the library does
 * not have OpenMP support. Otherwise, if \a nthreads=0, the
 * unthreaded version is used, if \a nthreads>0 it is treated as the
 ...
 *
 */
 gint wbfmm_bessel_h_init_f(gfloat x, gfloat *h0,
				gfloat *h1) {}
...
gint wbfmm_laplace_expansion_normal_cfft_f(gint N,
					       gfloat *x0,
					       gfloat *xs,
					       gfloat
					       *normal
...
gint wbfmm_laplace_expansion_evaluate_f(gfloat *x0,
					    gfloat *cfft,
					    gint cstr, gint N,
					    gint nq,
					    gfloat *xf,
...
gint wbfmm_laplace_expansion_grad_evaluate_f(gfloat *x0,
						 gfloat *cfft,
						 gint cstr,
						 gint N,
						 gint nq,
 ...
 * expansion to evaluate the Laplace potential. Evaluation
 * coefficients can be evaluated using
 * wbfmm_laplace_field_coefficients_f or
 * wbfmm_laplace_local_coefficients_f for the field (singular) or
 * local (regular) expansions respectively.
 * 
 * @param C coefficients of expansion;
 * @param cstr stride in \a C;
 ...
 * some combination of monopole and dipole sources. Source positions
 * are those in the point list attached to the tree using
 * ::wbfmm_tree_add_points_f and indexing in the array must
 * correspond to that in the point list. Input arrays may be NULL: if
 * \a src is not NULL, it is interpreted as a list of monopole
 * strengths; if \a normals is not NULL, \a dipoles may not be NULL
 * and they are interpreted respectively as a vector (`normal') at
...
gint wbfmm_laplace_field_grad_f(gfloat *xs,
				    gint xstride,
				    gfloat *src,
				    gint sstride,
				    gint nq,
...
gint wbfmm_laplace_field_curl_f(gfloat *xs,
				    gint xstride,
				    gfloat *src,
				    gint sstride,
				    gint nq,
...
gint
wbfmm_laplace_expansion_local_grad_evaluate_f(gfloat *x0,
						  gfloat *cfft,
						  gint cstr, 
						  gint N,
						  gint nq,
...
gint
wbfmm_laplace_expansion_local_curl_evaluate_f(gfloat *x0,
						  gfloat *cfft,
						  gint cstr, 
						  gint N,
						  gint nq,
...
gint
wbfmm_tree_laplace_box_local_curl_f(wbfmm_tree_t *t,
					guint level,
					guint b,
					gfloat *x,
					gfloat *f,
 ...
 *
 * The function is available as a reference version
 * wbfmm_laplace_downward_pass_ref_f and an optimized
 * version wbfmm_laplace_downward_pass_avx_f which
 * uses AVX optimizations if available. The compile time switch
 * -DWBFMM_USE_AVX selects the AVX version. The parameter \a nthreads
 * is ignored if the library does not have OpenMP support. Otherwise,
 * if \a nthreads=0, the unthreaded version is used, if \a nthreads>0
...
gint wbfmm_local_coefficients_f(gfloat k,
				    gfloat *x,
				    gint N,
				    guint field,
				    gfloat *cfft,
...
gint wbfmm_expansion_apply_f(gfloat *C,
				 gint cstr,
				 gint nq,
				 gfloat *ec,
				 gint N,
...
wbfmm_target_list_t *wbfmm_target_list_new_f(wbfmm_tree_t *t,
						 guint npts) ;
...
gint WBFMM_FUNCTION_NAME(wbfmm_laplace_field_direct)(gfloat *xs,
						     gint xstr,
						     gfloat *n,
						     gint nstr,
						     gint nsrc,
