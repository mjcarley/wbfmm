...

{
  ...
  WBFMM_REAL Cmch, Smch, Cnph, Snph, Cch, Sch, Cph, Sph, tr[32] = {0.0} ;
  WBFMM_REAL Hp, Hm ;
  ...
  for ( n = 0 ; n <= N ; n ++ ) {
	...
	Co[cstro*(idxo+1)+i] += Cnph*ti[i] - Snph*tr[i] ;
      }
    }
  }
...

{
  if ( nq == 1 ) 
    return
      _wbfmm_rotate_H_laplace_ref_1(Co, cstro, Ci, cstri, N, H, ph, ch) ;

  return
    _wbfmm_rotate_H_laplace_ref_nq(Co, cstro, Ci, cstri, N, nq, H, ph, ch) ;
