...
{
  WBFMM_REAL *Cr, *cr, *ci, H, H1, H2, tn[64] = {0.0}, c ;
  ...
  for ( n = 0 ; n <= Nc ; n ++ ) {
	...
	ci[2*nq+i] = buf[2*2*nq+2*i+0]*E0ph[1] - buf[2*2*nq+2*i+1]*E0ph[0] ;
	...
	ci[6*nq+i] = buf[6*2*nq+2*i+0]*E0ph[1] - buf[6*2*nq+2*i+1]*E0ph[0] ;
  ...
  for ( m = 1 ; m <= Np ; m ++ ) {
    for ( n = m ; n <= Np ; n ++ ) {
      ...
      for ( i = 0 ; i < 16*nq ; i ++ ) work[str*(ip+0)+i] = 0.0 ;
