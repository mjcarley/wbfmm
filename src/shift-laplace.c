...
static inline void increment_buf_pc_real(WBFMM_REAL *E0, WBFMM_REAL *E1,
					 WBFMM_REAL *C,
					 WBFMM_REAL H03p, WBFMM_REAL H03m,
					 WBFMM_REAL H47p, WBFMM_REAL H47m,
					 gint nq,
...

{
  ...
  for ( i = 0 ; i < nq ; i ++ ) {
    buf[7*nq+i] += (C[0*nq+i]*E0[0] - C[8*nq+0*nq+i]*E0[1])*H47 ;
    buf[6*nq+i] += (C[0*nq+i]*E1[0] - C[8*nq+0*nq+i]*E1[1])*H47 ;
    buf[5*nq+i] += (C[0*nq+i]*E0[0] + C[8*nq+0*nq+i]*E0[1])*H47 ;
    buf[4*nq+i] += (C[0*nq+i]*E1[0] + C[8*nq+0*nq+i]*E1[1])*H47 ;

    buf[3*nq+i] += (C[0*nq+i]*E0[0] - C[8*nq+0*nq+i]*E0[1])*H03 ;
    buf[2*nq+i] += (C[0*nq+i]*E1[0] - C[8*nq+0*nq+i]*E1[1])*H03 ;
    buf[1*nq+i] += (C[0*nq+i]*E0[0] + C[8*nq+0*nq+i]*E0[1])*H03 ;
    buf[0*nq+i] += (C[0*nq+i]*E1[0] + C[8*nq+0*nq+i]*E1[1])*H03 ;
  }

  return ;
}

static inline void increment_buf_pc_complex(WBFMM_REAL *E0, WBFMM_REAL *E1,
					    WBFMM_REAL *C,
					    WBFMM_REAL H03p, WBFMM_REAL H03m,
					    WBFMM_REAL H47p, WBFMM_REAL H47m,
					    gint nq,
...

{
  ...
  dH03 = H03p - H03m ; dH47 = H47p - H47m ;
  for ( i = 0 ; i < nq ; i ++ ) {
    buf[7*2*nq+2*i+0] += (C[0*nq+i]*E0[0] - C[8*nq+0*nq+i]*E0[1])*sH47 ;
    buf[7*2*nq+2*i+1] += (C[0*nq+i]*E0[1] + C[8*nq+0*nq+i]*E0[0])*dH47 ;

    buf[6*2*nq+2*i+0] += (C[0*nq+i]*E1[0] - C[8*nq+0*nq+i]*E1[1])*sH47 ;
    buf[6*2*nq+2*i+1] += (C[0*nq+i]*E1[1] + C[8*nq+0*nq+i]*E1[0])*dH47 ;

    buf[5*2*nq+2*i+0] += (C[0*nq+i]*E0[0] + C[8*nq+0*nq+i]*E0[1])*sH47 ;
    buf[5*2*nq+2*i+1] += (C[0*nq+i]*E0[1] - C[8*nq+0*nq+i]*E0[0])*dH47 ;

    buf[4*2*nq+2*i+0] += (C[0*nq+i]*E1[0] + C[8*nq+0*nq+i]*E1[1])*sH47 ;
    buf[4*2*nq+2*i+1] -= (C[0*nq+i]*E1[1] - C[8*nq+0*nq+i]*E1[0])*dH47 ;

    buf[3*2*nq+2*i+0] += (C[0*nq+i]*E0[0] - C[8*nq+0*nq+i]*E0[1])*sH03 ;
    buf[3*2*nq+2*i+1] += (C[0*nq+i]*E0[1] + C[8*nq+0*nq+i]*E0[0])*dH03 ;

    buf[2*2*nq+2*i+0] += (C[0*nq+i]*E1[0] - C[8*nq+0*nq+i]*E1[1])*sH03 ;
    buf[2*2*nq+2*i+1] += (C[0*nq+i]*E1[1] + C[8*nq+0*nq+i]*E1[0])*dH03 ;

    buf[1*2*nq+2*i+0] += (C[0*nq+i]*E0[0] + C[8*nq+0*nq+i]*E0[1])*sH03 ;
    buf[1*2*nq+2*i+1] += (C[0*nq+i]*E0[1] - C[8*nq+0*nq+i]*E0[0])*dH03 ;

    buf[0*2*nq+2*i+0] += (C[0*nq+i]*E1[0] + C[8*nq+0*nq+i]*E1[1])*sH03 ;
    buf[0*2*nq+2*i+1] -= (C[0*nq+i]*E1[1] - C[8*nq+0*nq+i]*E1[0])*dH03 ;
  }

  return ;
}

static inline void increment_cfft_pc_real(WBFMM_REAL *E0, WBFMM_REAL *E1,
					  WBFMM_REAL *C,
					  WBFMM_REAL H03p, WBFMM_REAL H03m,
					  WBFMM_REAL H47p, WBFMM_REAL H47m,
					  gint nq,
...

{
  ...
  for ( i = 0 ; i < nq ; i ++ ) {
    buf[7*nq+i] += (C[7*nq+i]*E0[0] - C[8*nq+7*nq+i]*E0[1])*H47 ;
    buf[6*nq+i] += (C[6*nq+i]*E1[0] - C[8*nq+6*nq+i]*E1[1])*H47 ;
    buf[5*nq+i] += (C[5*nq+i]*E0[0] + C[8*nq+5*nq+i]*E0[1])*H47 ;
    buf[4*nq+i] += (C[4*nq+i]*E1[0] + C[8*nq+4*nq+i]*E1[1])*H47 ;
    
    buf[3*nq+i] += (C[3*nq+i]*E0[0] - C[8*nq+3*nq+i]*E0[1])*H03 ;
    buf[2*nq+i] += (C[2*nq+i]*E1[0] - C[8*nq+2*nq+i]*E1[1])*H03 ;
    buf[1*nq+i] += (C[1*nq+i]*E0[0] + C[8*nq+1*nq+i]*E0[1])*H03 ;
    buf[0*nq+i] += (C[0*nq+i]*E1[0] + C[8*nq+0*nq+i]*E1[1])*H03 ;
  }
...
static inline void increment_cfft_pc_complex(WBFMM_REAL *E0, WBFMM_REAL *E1,
					     WBFMM_REAL *C,
					     WBFMM_REAL H03p, WBFMM_REAL H03m,
					     WBFMM_REAL H47p, WBFMM_REAL H47m,
					     gint nq,
...

{
  ...
  dH03 = H03p - H03m ; dH47 = H47p - H47m ;
  for ( i = 0 ; i < nq ; i ++ ) {
    buf[7*2*nq+2*i+0] += (C[7*nq+i]*E0[0] - C[8*nq+7*nq+i]*E0[1])*sH47 ;
    buf[7*2*nq+2*i+1] += (C[7*nq+i]*E0[1] + C[8*nq+7*nq+i]*E0[0])*dH47 ;

    buf[6*2*nq+2*i+0] += (C[6*nq+i]*E1[0] - C[8*nq+6*nq+i]*E1[1])*sH47 ;
    buf[6*2*nq+2*i+1] += (C[6*nq+i]*E1[1] + C[8*nq+6*nq+i]*E1[0])*dH47 ;

    buf[5*2*nq+2*i+0] += (C[5*nq+i]*E0[0] + C[8*nq+5*nq+i]*E0[1])*sH47 ;
    buf[5*2*nq+2*i+1] += (C[5*nq+i]*E0[1] - C[8*nq+5*nq+i]*E0[0])*dH47 ;

    buf[4*2*nq+2*i+0] += (C[4*nq+i]*E1[0] + C[8*nq+4*nq+i]*E1[1])*sH47 ;
    buf[4*2*nq+2*i+1] -= (C[4*nq+i]*E1[1] - C[8*nq+4*nq+i]*E1[0])*dH47 ;

    buf[3*2*nq+2*i+0] += (C[3*nq+i]*E0[0] - C[8*nq+3*nq+i]*E0[1])*sH03 ;
    buf[3*2*nq+2*i+1] += (C[3*nq+i]*E0[1] + C[8*nq+3*nq+i]*E0[0])*dH03 ;

    buf[2*2*nq+2*i+0] += (C[2*nq+i]*E1[0] - C[8*nq+2*nq+i]*E1[1])*sH03 ;
    buf[2*2*nq+2*i+1] += (C[2*nq+i]*E1[1] + C[8*nq+2*nq+i]*E1[0])*dH03 ;

    buf[1*2*nq+2*i+0] += (C[1*nq+i]*E0[0] + C[8*nq+1*nq+i]*E0[1])*sH03 ;
    buf[1*2*nq+2*i+1] += (C[1*nq+i]*E0[1] - C[8*nq+1*nq+i]*E0[0])*dH03 ;
...

{
  ...
  for ( n = 0 ; n <= Np ; n ++ ) {
    ...
    /*input is single parent coefficient which must be rotated eight
      ways*/
    H = H03[wbfmm_rotation_index_numn(nu,m,n)] ;
    for ( i = 0 ; i < nq ; i ++ ) {
	...
	ip = wbfmm_index_laplace_nm(n,m) ;
	increment_buf_pc_complex(E0, E1, &(Cp[str*ip]),
				 H03[wbfmm_rotation_index_numn( nu,m,n)],
				 H03[wbfmm_rotation_index_numn(-nu,m,n)],
				 H47[wbfmm_rotation_index_numn( nu,m,n)],
				 H47[wbfmm_rotation_index_numn(-nu,m,n)],
      ...
      for ( i = 0 ; i < nq ; i ++ ) {
	cr[7*nq+i] += buf[0*2*nq+2*i+0]*E0ph[0] - buf[0*2*nq+2*i+1]*E0ph[1] ;
	ci[7*nq+i] += buf[0*2*nq+2*i+1]*E0ph[0] + buf[0*2*nq+2*i+0]*E0ph[1] ;

	cr[6*nq+i] += buf[1*2*nq+2*i+0]*E0ph[0] + buf[1*2*nq+2*i+1]*E0ph[1] ;
	ci[6*nq+i] += buf[1*2*nq+2*i+1]*E0ph[0] - buf[1*2*nq+2*i+0]*E0ph[1] ;

	cr[5*nq+i] += buf[2*2*nq+2*i+0]*E0ph[0] + buf[2*2*nq+2*i+1]*E0ph[1] ;
	ci[5*nq+i] -= buf[2*2*nq+2*i+1]*E0ph[0] - buf[2*2*nq+2*i+0]*E0ph[1] ;

	cr[4*nq+i] += buf[3*2*nq+2*i+0]*E0ph[0] - buf[3*2*nq+2*i+1]*E0ph[1] ;
	ci[4*nq+i] += buf[3*2*nq+2*i+1]*E0ph[0] + buf[3*2*nq+2*i+0]*E0ph[1] ;

	cr[3*nq+i] += buf[4*2*nq+2*i+0]*E0ph[0] - buf[4*2*nq+2*i+1]*E0ph[1] ;
	ci[3*nq+i] += buf[4*2*nq+2*i+1]*E0ph[0] + buf[4*2*nq+2*i+0]*E0ph[1] ;

	cr[2*nq+i] += buf[5*2*nq+2*i+0]*E0ph[0] + buf[5*2*nq+2*i+1]*E0ph[1] ;
	ci[2*nq+i] += buf[5*2*nq+2*i+1]*E0ph[0] - buf[5*2*nq+2*i+0]*E0ph[1] ;

	cr[1*nq+i] += buf[6*2*nq+2*i+0]*E0ph[0] + buf[6*2*nq+2*i+1]*E0ph[1] ;
	ci[1*nq+i] -= buf[6*2*nq+2*i+1]*E0ph[0] - buf[6*2*nq+2*i+0]*E0ph[1] ;

	cr[0*nq+i] += buf[7*2*nq+2*i+0]*E0ph[0] - buf[7*2*nq+2*i+1]*E0ph[1] ;
	ci[0*nq+i] += buf[7*2*nq+2*i+1]*E0ph[0] + buf[7*2*nq+2*i+0]*E0ph[1] ;
      }
    }
  }
  ...
  /* memcpy(Cc, work, 256*nq*sizeof(WBFMM_REAL)) ; */
  ...
  for ( n = 0 ; n <= Nc ; n ++ ) {
	...
	ip = wbfmm_index_laplace_nm(n,m) ;
	increment_cfft_pc_complex(E0, E1, &(work[str*ip]),
				  H03[wbfmm_rotation_index_numn( nu,m,n)],
				  H03[wbfmm_rotation_index_numn(-nu,m,n)],
				  H47[wbfmm_rotation_index_numn( nu,m,n)],
				  H47[wbfmm_rotation_index_numn(-nu,m,n)],
