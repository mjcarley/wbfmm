...

{
  ...
  gdouble D, xtree[3] = {0.0}, xtmax[3], *sources ;
  ...
  f = (gdouble *)g_malloc0(nf*nq*sizeof(gdouble)) ;

  if ( fit_box ) {
    wbfmm_points_origin_width(sources, str, nsrc, xtree, xtmax, &D, TRUE) ;
  ...
  wbfmm_tree_add_points(tree, (gpointer)sources, nsrc, pstr) ;

  for ( i = 0 ; i < depth ; i ++ ) wbfmm_tree_refine(tree) ;

  /* if ( write_sources ) { */
  /*   wbfmm_tree_write_sources(tree, &(sources[3]), str, stderr) ; */
  ...
  fprintf(stderr, "%s: initializing target point list; %lg\n",
	  progname, g_timer_elapsed(timer, NULL)) ;
  targets = wbfmm_target_list_new(tree, nf, FALSE) ;
  ...
  {
    gdouble xf[3], phi[16] = {0.0} ;

    xf[0] = 4.1 ; xf[1] = 0.4 ; xf[2] = -0.7 ;
    level = 2 ;
    
    wbfmm_laplace_box_fields(tree, level, xf, phi, work) ;
    
    fprintf(stdout, "%lg %lg %lg", xf[0], xf[1], xf[2]) ;
    for ( i = 0 ; i < nq ; i ++ ) 
  ...
  wbfmm_target_list_local_field(targets, q, qstr, f) ;

  /* for ( i = 0 ; i < nf ; i ++ ) { */
  /*   b = wbfmm_point_box(tree, level, &(xf[i*strf])) ; */
  /*   wbfmm_tree_laplace_box_local_field(tree, level, b, */
  /* 					    &(xf[i*strf]), &(f[nq*i]), */
  /* 					    q, qstr, normals, nstr, */
  /* 					    dipoles, dstr, */
  /* 					    TRUE, work) ; */
