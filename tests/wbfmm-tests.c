...

{
  ...
  for ( i = 0 ; i < nf ; i ++ ) {
    ...
    fprintf(stdout, "%lg+j*%lg ", fs[0], fs[1]) ;

    wbfmm_total_field(k, xs, sstr, q, qstr,
			   NULL, 0, NULL, 0, nq, 
			   ns, &(xf[i*fstr]), fc, 1) ;

    fprintf(stdout, "%lg+j*%lg ", fc[0], fc[1]) ;
...

{
  ...
  gdouble f0[64] = {0.0}, ft[64] = {0.0}, fc[64] = {0.0} ;
  gint i, Ni, No, cstri, cstro ;
  gdouble t0 ;
  ...
  /*compute field in both frames*/
  wbfmm_expansion_h_evaluate(k, x0, Ci, cstri, Ni, nq, xr, f0, 2, work) ;

  fprintf(stdout, "%lg+j*%lg ", f0[0], f0[1]) ;

  wbfmm_expansion_j_evaluate(k, xt, Co, cstro, No, nq, xr, ft, work) ;

  fprintf(stdout, "%lg+j*%lg ", ft[0], ft[1]) ;
		     ...
		     
{
  ...
  wbfmm_rotate_H(Co, cstro, Ci, cstri, N, nq, H, ph, ch) ;
  ...
  for ( i = 0 ; i < nf ; i ++ ) {
    ...
    fprintf(stdout, "%lg+j*%lg ", fu[0], fu[1]) ;

    wbfmm_coordinate_transform(&(xf[i*fstr]), ix, iy, iz, y) ;

    wbfmm_expansion_h_evaluate(k, y0, Co, cstro, N, nq, y, fr, 2, work) ;

    fprintf(stdout, "%lg+j*%lg ", fr[0], fr[1]) ;
...

{
  ...
  fprintf(stderr, "%s child expansion generated: %lg\n",
	  __FUNCTION__, g_timer_elapsed(timer, NULL) - t0) ;
  wbfmm_coaxial_translate(&(parent[2*pq]), 8, Np, child, 1, Np, nq,
			       SRshift, Np, TRUE) ;
  ...
  for ( i = 0 ; i < 8 ; i ++ ) {
    ...
    wbfmm_total_field(k, xsrc, 3, src, 2, NULL, 0, NULL, 0, nq,
			   1, xf, fe, 1) ;
    wbfmm_expansion_j_evaluate(k, x0, &(parent[2*pq]), 8, Np, nq,
				    xf, fp, work) ;
    wbfmm_expansion_j_evaluate(k, &(xb[3*i]), &(child[2*i]), 8, Nc, nq,
				    xf, fc, work) ;
  ...
  
{
  ...
  /*apply the rotation to the coefficients*/
  wbfmm_rotate_H(Ca, cstr0, C0, cstr0, N0, nq, H0, ph0, ch0) ;
  ...
  fprintf(stderr, "%s expansion coefficients translated: %lg\n",
	  __FUNCTION__, g_timer_elapsed(timer, NULL) - t0) ;
  /*rotate back*/
  wbfmm_rotate_H(C1, cstr1, Cb, cstr1, N1, nq, H0, ch0, ph0) ;
  fprintf(stderr, "%s expansion coefficients shifted: %lg\n",
	  __FUNCTION__, g_timer_elapsed(timer, NULL) - t0) ;
  ...
  wbfmm_total_field(k, xs, sstr, q, qstr, NULL, 0, NULL, 0, nq,
			 ns, xr, fc, 1) ;
  /*computed field on unshifted coefficients*/
  wbfmm_expansion_h_evaluate(k, x0, C0, cstr0, N0, nq, xr, f0, 2, work) ;
  
  fprintf(stdout, "%lg+j*%lg ", f0[0], f0[1]) ;
  
  /*computed field on shifted coefficients*/
  wbfmm_expansion_j_evaluate(k, x1, C1, cstr1, N1, nq, xr, f1, work) ;
