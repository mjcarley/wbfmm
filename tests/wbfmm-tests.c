		     ...
		     
{
  ...
  /* Ci = (gdouble *)g_malloc0(2*cstri*wbfmm_coefficient_index_nm(N+1,-N-1)* */
  /* 			       sizeof(gdouble)) ; */
  /* Co = (gdouble *)g_malloc0(2*cstro*wbfmm_coefficient_index_nm(N+1,-N-1)* */
  /* 			       sizeof(gdouble)) ; */
  Ci = (gdouble *)g_malloc0(2*cstri*wbfmm_coefficient_number(N)*
			       sizeof(gdouble)) ;
  Co = (gdouble *)g_malloc0(2*cstro*wbfmm_coefficient_number(N)*
			       sizeof(gdouble)) ;
  ...
  fprintf(stderr, "%s AVX rotation: %lg\n",
	  __FUNCTION__, (dt = g_timer_elapsed(timer, NULL)) - t0) ;
  wbfmm_rotate_H_avx(Co, cstro, Ci, cstri, N, nq, H, ph, ch, 0.0) ;
  dt = g_timer_elapsed(timer, NULL) - dt ;
  fprintf(stderr, "%s rotation complete: %lg (%lg)\n",
	  __FUNCTION__, g_timer_elapsed(timer, NULL) - t0, dt) ;
#endif /*WBFMM_USE_AVX*/
...
static gint rotation_matrix_test(gdouble *x0, gdouble *x1, gdouble *x2,
				 gdouble *ix, gdouble *iy, gdouble *iz,
				 gdouble k,
				 gdouble *xs, gint sstr, gint ns,
				 gdouble *q , gint qstr, gint nq,
		     ...
		     
{
  ...
  wbfmm_coefficients_H_rotation(H, N, th, work) ;
  T = (gdouble *)g_malloc(2*wbfmm_T_rotation_matrix_size(N)*
  			     sizeof(gdouble)) ;

  wbfmm_coefficients_H_to_T(H, N, th, ph, ch, T) ;
  
  /*apply the T rotation to the coefficients*/
  fprintf(stderr, "%s reference rotation: %lg\n",
	  __FUNCTION__, (dt = g_timer_elapsed(timer, NULL)) - t0) ;
  /* wbfmm_rotate_H(Co, cstro, Ci, cstri, N, nq, H, ph, ch, 0.0) ; */
  wbfmm_rotate_T(Co, cstro, Ci, cstri, N, nq, T, bt) ;

  dt = g_timer_elapsed(timer, NULL) - dt ;
  fprintf(stderr, "%s rotation complete: %lg (%lg)\n",
	  __FUNCTION__, g_timer_elapsed(timer, NULL) - t0, dt) ;

  /* wbfmm_rotate_T(Cc, cstro, Ci, cstri, N, nq, T, bt) ; */
  wbfmm_coordinate_transform(x0, ix, iy, iz, y0) ;
...

{
  ...
  while ( (ch = getopt(argc, argv, "cef:i:k:l:n:N:q:t:w:x:")) != EOF ) {
    switch ( ch ) {
    default: g_assert_not_reached() ; break ;
    case 'c':
