...

{
  ...
  if ( fit_box ) {
    ...
    wbfmm_points_origin_width_f(xf, fstr, nf, xtree, xtmax, &D, FALSE) ;
  ...
  work = (gfloat *)g_malloc0(nproc*sizew*sizeof(gfloat)) ;
  ...
  if ( write_sources ) {
    wbfmm_tree_write_sources_f(tree, &(xs[3]), xstr, stderr) ;
  ...
  wbfmm_target_list_add_points_f(targets, xf, fstr*sizeof(gfloat), nf) ;
  /* wbfmm_target_list_add_points_f(targets, xf, fstr*sizeof(gfloat), */
  /* 				    NULL, 0, nf) ; */
  ...
  for ( level = 2 ; level <= depth ; level ++ ) {
    wbfmm_downward_pass_f(tree, shifts, level, work, nthreads) ;
  }
