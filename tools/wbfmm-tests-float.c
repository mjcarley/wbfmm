...
static gint check_isnan(char *name, gfloat *f, gint n)
...

{
  ...
  gfloat kr, work[BUFSIZE], xr[3], xt[3] ;
  gfloat eval[BUFSIZE] = {0.0} ;  
  gfloat fl[32]={0.0}, fe[32]={0.0}, fc[32]={0.0}, fp[32] = {0.0} ;
  ...
  fprintf(stderr, "shifted expansion: %g %g %g\n", 
	  xt[0], xt[1], xt[2]) ;
  fprintf(stderr, "local field point: %g %g %g\n", 
	  xr[0], xr[1], xr[2]) ;
  fprintf(stderr, "coaxial translation: kr = %g\n", kr) ;
  ...
  xr[0] -= xt[0] ; xr[1] -= xt[1] ; xr[2] -= xt[2] ; 
  wbfmm_local_coefficients_f(k, xr, N, field, eval, work) ;
  /* memset(fe, 0, 6*sizeof(gfloat)) ; */
  wbfmm_expansion_apply_f(Co, cstro, nq, eval, No, field, fp, fstr) ;
  ...
  for ( i = 0 ; i < nq ; i ++ ) {
    ...
    fprintf(stdout, "pre-computed: %g+j*%g "
	    "%g+j*%g %g+j*%g\n",
...

{
  ...
  wbfmm_tree_add_points_f(t, (gpointer)pts, pstr, NULL, 0,
			     npts, sort_points) ;
